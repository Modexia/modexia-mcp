...
#### Cross-Chain Transfer (CCTP & Squid Router)
Send USDC natively to another blockchain. The platform uses Squid Router to abstract bridge operations and gas relaying on the destination chain, making it a frictionless 1-step experience.

```python
receipt = client.cross_chain_transfer(
    to_chain="akashnet-2", # Target chain ID (e.g. '1' for ETH, '42161' for ARB, 'akashnet-2' for Akash)
    to_token="0xUSDC...",  # USDC contract address on destination chain
    recipient="akash1...", # Target wallet address
    amount=15.00,
    idempotency_key="task_cctp_123"
)
# Returns PENDING status immediately while Squid routes the funds asynchronously.
```
